...
function timespan (citySlug, startDate, duration) {
  ...
  return {
    ...
    dataRequest: {
      requestString: 'query($citySlug:ID!,$first:Date!,$after:Date!){city(slug:$citySlug){slug name tags{slug title} firstDate days(first:$first,after:$after){date events{time title mainTag{slug} tags{slug} artist{name} location{name} occurences{start}}}}}',
      variableValues: {
        citySlug,
